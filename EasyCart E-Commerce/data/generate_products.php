...
function generateProducts($category_id, $templates, $brand_ids, $adjectives, $qualities, $brands, $countries) {
    $products = [];
    ...
    for ($i = 0; $i < 100; $i++) {
        ...
        $product = [
            ...
            "stock" => rand(0, 100),
            "description" => "High-quality " . strtolower($template['name']) . " with excellent features and performance",
            "long_description" => "Experience the best in class " . strtolower($template['name']) . " with advanced technology and superior craftsmanship. Perfect for your needs.",
            "features" => [
            ...
            "specifications" => [
                "Model" => sprintf("PRD-%04d", 0),
                "Brand" => $brands[$brand_id],
                "Warranty" => rand(1, 3) . " Year(s)",
                "Made In" => $countries[array_rand($countries)]
            ],
            "variants" => [
                "color" => ["Black", "White", "Blue", "Red", "Gray"],
                "size" => ($category_id == 2 || $category_id == 4) ? ["S", "M", "L", "XL"] : []
            ],
            "icon" => $template['icon'],
            "new" => rand(0, 10) > 7,
...
$electronics = generateProducts(1, $electronics_products, [1, 6, 8], $adjectives, $qualities, $brands, $countries);
foreach ($electronics as $product) {
    $product['id'] = $product_id;
    $product['slug'] = $product['slug'] . '-' . $product_id;
    $product['specifications']['Model'] = sprintf("EL-%04d", $product_id);
    $all_products[$product_id] = $product;
    $product_id++;
}

// Category 2: Fashion
$fashion = generateProducts(2, $fashion_products, [2, 7, 9], $adjectives, $qualities, $brands, $countries);
foreach ($fashion as $product) {
    $product['id'] = $product_id;
    $product['slug'] = $product['slug'] . '-' . $product_id;
    $product['specifications']['Model'] = sprintf("FA-%04d", $product_id);
    $all_products[$product_id] = $product;
    $product_id++;
}

// Category 3: Home & Living
$home = generateProducts(3, $home_products, [3, 10], $adjectives, $qualities, $brands, $countries);
foreach ($home as $product) {
    $product['id'] = $product_id;
    $product['slug'] = $product['slug'] . '-' . $product_id;
    $product['specifications']['Model'] = sprintf("HM-%04d", $product_id);
    $all_products[$product_id] = $product;
    $product_id++;
}

// Category 4: Sports
$sports = generateProducts(4, $sports_products, [4, 11], $adjectives, $qualities, $brands, $countries);
foreach ($sports as $product) {
    $product['id'] = $product_id;
    $product['slug'] = $product['slug'] . '-' . $product_id;
    $product['specifications']['Model'] = sprintf("SP-%04d", $product_id);
    $all_products[$product_id] = $product;
    $product_id++;
}

// Category 5: Books
$books = generateProducts(5, $books_products, [5, 12], $adjectives, $qualities, $brands, $countries);
foreach ($books as $product) {
    $product['id'] = $product_id;
    $product['slug'] = $product['slug'] . '-' . $product_id;
    $product['specifications']['Model'] = sprintf("BK-%04d", $product_id);
    $product['variants']['size'] = ["Paperback", "Hardcover"];
    $all_products[$product_id] = $product;
    $product_id++;
}

// Save to JSON file
file_put_contents('products.json', json_encode($all_products, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

echo "Generated " . count($all_products) . " products successfully!\n";
?>
